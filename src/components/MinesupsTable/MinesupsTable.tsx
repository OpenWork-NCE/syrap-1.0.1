...
const useGetMinesups = () => {
	return useQuery<MinesupApiResponse>({
		queryKey: ["minsup"],
		queryFn: () => fetch(innerUrl("/api/minsup")).then((res) => res.json()),
...
function useCreateMinesup() {
	...
	return useMutation({
		...
		onMutate: (newMinesupInfo: Minesup) => {
			queryClient.setQueryData(["minsup"], (prevMinesups: any) => {
		...
		onSettled: () => {
			queryClient.invalidateQueries({ queryKey: ["minsup"] });
		},
	});
}
...
function useUpdateMinesup() {
	...
	return useMutation({
		mutationFn: async (minesup: Minesup) => {
			const response = await fetch(
				innerUrl(`/api/minsup/${minesup.id}/update`),
				{
					method: "PUT",
					headers: {
						"Content-Type": "application/json",
...
function useDeleteMinesup() {
	...
	return useMutation({
		mutationFn: async (minesupId: string) => {
			const response = await fetch(
				innerUrl(`/api/minsup/${minesupId}/delete`),
				{
					method: "DELETE",
					headers: {
						"Content-Type": "application/json",
		...
		onMutate: (minesupId: string) => {
			queryClient.cancelQueries({ queryKey: ["minsup"] });

			const previousMinesups = queryClient.getQueryData(["minsup"]);
		...
		onError: (err, minesupId, context: any) => {
			if (context?.previousMinesups) {
				queryClient.setQueryData(["minsup"], context.previousMinesups);
			}
		},
		onSettled: () => {
			queryClient.invalidateQueries({ queryKey: ["minsup"] });
